...
REM ── Sanity check: node.exe and node_modules must be present in this folder ──
...
REM Kill Chrome/Edge that might be stuck on an old session
...
REM ── Clear Chrome cache (so updates appear immediately) ────────────────
...
REM ── Start the local server with restart-after-update loop ─────────────
...
REM ── Watch loop: restart browser if closed, check server health ────────
set HEALTH_FAIL=0
:WATCH_LOOP
timeout /t 15 /nobreak >nul

REM Check if browser is still running
...
REM Check server health endpoint
...
REM 3 consecutive failures: server is down, kill it so restart-loop relaunches
...
REM ── Browser closed: wait for server health before relaunching ─────────
